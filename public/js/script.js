...
document.addEventListener('click', (e) => {
  ...
  if (!anyPanelActive && !sidebar.contains(e.target) && e.target !== menuBtn) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  dropdownButtons.forEach(button => {
    button.addEventListener('click', e => {
      ...
      const sourceDropdown = document.querySelector('.dropdown-content');
...
document.querySelectorAll('.see-more').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    let sisa = 1;
    if (window.roomAvailability && window.roomAvailability[roomName] !== undefined) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", async function (e) {
        ...
        try {
            const response = await fetch(`/pesma/public/cek-kamar?checkin=${checkin}&checkout=${checkout}`);
            const data = await response.json();

            window.roomAvailability = data;

            for (const [roomName, sisa] of Object.entries(data)) {
                const el = document.querySelector(`.availability-text[data-room="${roomName}"]`);
                if (!el) continue;
